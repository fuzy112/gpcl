...
inline int GPCL_DETAIL_TLSF_FLS_SIZET(size_t size)
{
...
#else
#define GPCL_DETAIL_TLSF_FLS_SIZET tlsf_fls
#endif
...
#define GPCL_DETAIL_TLSF_CAST(t, exp)	((t)(exp))
#define GPCL_DETAIL_TLSF_MIN(a, b)		((a) < (b) ? (a) : (b))
#define GPCL_DETAIL_TLSF_MAX(a, b)		((a) > (b) ? (a) : (b))
...
*/
constexpr const size_t block_header_free_bit = 1 << 0;
constexpr const size_t block_header_prev_free_bit = 1 << 1;

/*
** The size of the block header exposed to used blocks is the size field.
** The prev_phys_block field is stored *inside* the previous free block.
*/
constexpr const size_t block_header_overhead = sizeof(size_t);

/* User data starts directly after the size field in a used block. */
constexpr const size_t block_start_offset =
...
** bits for FL_INDEX.
*/
constexpr const size_t block_size_min = 
	sizeof(block_header_t) - sizeof(block_header_t*);
constexpr const size_t block_size_max = GPCL_DETAIL_TLSF_CAST(size_t, 1) << FL_INDEX_MAX;
...
GPCL_DECL size_t block_size(const block_header_t* block)
{
	return block->size & ~(block_header_free_bit | block_header_prev_free_bit);
}

GPCL_DECL void block_set_size(block_header_t* block, size_t size)
{
	const size_t oldsize = block->size;
	block->size = size | (oldsize & (block_header_free_bit | block_header_prev_free_bit));
}

GPCL_DECL int block_is_last(const block_header_t* block)
{
	return block_size(block) == 0;
}

GPCL_DECL int block_is_free(const block_header_t* block)
{
	return GPCL_DETAIL_TLSF_CAST(int, block->size & block_header_free_bit);
}

GPCL_DECL void block_set_free(block_header_t* block)
{
	block->size |= block_header_free_bit;
}

GPCL_DECL void block_set_used(block_header_t* block)
{
	block->size &= ~block_header_free_bit;
}

GPCL_DECL int block_is_prev_free(const block_header_t* block)
{
	return GPCL_DETAIL_TLSF_CAST(int, block->size & block_header_prev_free_bit);
}

GPCL_DECL void block_set_prev_free(block_header_t* block)
{
	block->size |= block_header_prev_free_bit;
}

GPCL_DECL void block_set_prev_used(block_header_t* block)
{
	block->size &= ~block_header_prev_free_bit;
}

GPCL_DECL block_header_t* block_from_ptr(const void* ptr)
{
	return GPCL_DETAIL_TLSF_CAST(block_header_t*,
		GPCL_DETAIL_TLSF_CAST(unsigned char*, ptr) - block_start_offset);
}

GPCL_DECL void* block_to_ptr(const block_header_t* block)
{
	return GPCL_DETAIL_TLSF_CAST(void*,
		GPCL_DETAIL_TLSF_CAST(unsigned char*, block) + block_start_offset);
}

/* Return location of next block after block of given size. */
GPCL_DECL block_header_t* offset_to_block(const void* ptr, size_t size)
{
	return GPCL_DETAIL_TLSF_CAST(block_header_t*, GPCL_DETAIL_TLSF_CAST(tlsfptr_t, ptr) + size);
}
...
GPCL_DECL block_header_t* block_next(const block_header_t* block)
{
...
GPCL_DECL block_header_t* block_link_next(block_header_t* block)
{
...
GPCL_DECL void block_mark_as_free(block_header_t* block)
{
...
GPCL_DECL void block_mark_as_used(block_header_t* block)
{
	block_header_t* next = block_next(block);
...
GPCL_DECL void* align_ptr(const void* ptr, size_t align)
{
...
*/
GPCL_DECL size_t adjust_request_size(size_t size, size_t align)
{
	size_t adjust = 0;
	if (size)
	{
		...
		{
			adjust = GPCL_DETAIL_TLSF_MAX(aligned, block_size_min);
		}
	}
	return adjust;
}
...
GPCL_DECL void mapping_insert(size_t size, int* fli, int* sli)
{
	int fl, sl;
	if (size < SMALL_BLOCK_SIZE)
	{
		...
		fl = 0;
		sl = GPCL_DETAIL_TLSF_CAST(int, size) / (SMALL_BLOCK_SIZE / SL_INDEX_COUNT);
	}
	else
	{
		fl = GPCL_DETAIL_TLSF_FLS_SIZET(size);
		sl = GPCL_DETAIL_TLSF_CAST(int, size >> (fl - SL_INDEX_COUNT_LOG2)) ^ (1 << SL_INDEX_COUNT_LOG2);
		fl -= (FL_INDEX_SHIFT - 1);
	}
	*fli = fl;
	*sli = sl;
...
GPCL_DECL void mapping_search(size_t size, int* fli, int* sli)
{
	if (size >= SMALL_BLOCK_SIZE)
	{
		const size_t round = (1 << (GPCL_DETAIL_TLSF_FLS_SIZET(size) - SL_INDEX_COUNT_LOG2)) - 1;
		size += round;
	}
...
GPCL_DECL int block_can_split(block_header_t* block, size_t size)
{
	return block_size(block) >= sizeof(block_header_t) + size;
}

/* Split a block into two, the second of which is free. */
GPCL_DECL block_header_t* block_split(block_header_t* block, size_t size)
{
...
GPCL_DECL void integrity_walker(void* ptr, size_t size, int used, void* user)
{
	...
	integrity_t* integ = GPCL_DETAIL_TLSF_CAST(integrity_t*, user);
	...
	int status = 0;
	(void)used;
	GPCL_DETAIL_TLSF_INSIST(integ->prev_status == this_prev_status && "prev status incorrect");
	GPCL_DETAIL_TLSF_INSIST(size == this_block_size && "block size incorrect");
...
int tlsf_check(tlsf_t tlsf)
{
	...
	for (i = 0; i < FL_INDEX_COUNT; ++i)
	{
		for (j = 0; j < SL_INDEX_COUNT; ++j)
		{
			...
			if (!fl_map)
			{
				GPCL_DETAIL_TLSF_INSIST(!sl_map && "second-level map must be null");
			}

			if (!sl_map)
			{
				GPCL_DETAIL_TLSF_INSIST(block == &control->block_null && "block list must be null");
				continue;
			}

			/* Check that there is at least one free block. */
			GPCL_DETAIL_TLSF_INSIST(sl_list && "no free blocks in second-level map");
			GPCL_DETAIL_TLSF_INSIST(block != &control->block_null && "block should not be null");

			while (block != &control->block_null)
			{
				int fli, sli;
				GPCL_DETAIL_TLSF_INSIST(block_is_free(block) && "block should be free");
				GPCL_DETAIL_TLSF_INSIST(!block_is_prev_free(block) && "blocks should have coalesced");
				GPCL_DETAIL_TLSF_INSIST(!block_is_free(block_next(block)) && "blocks should have coalesced");
				GPCL_DETAIL_TLSF_INSIST(block_is_prev_free(block_next(block)) && "block should be free");
				GPCL_DETAIL_TLSF_INSIST(block_size(block) >= block_size_min && "block not minimum size");

				mapping_insert(block_size(block), &fli, &sli);
				GPCL_DETAIL_TLSF_INSIST(fli == i && sli == j && "block size indexed in wrong list");
...
#undef GPCL_DETAIL_TLSF_INSIST

GPCL_DECL void default_walker(void* ptr, size_t size, int used, void* user)
{
	(void)user;
...
#if defined (GPCL_DETAIL_TLSF_64BIT)
	rv += (GPCL_DETAIL_TLSF_FLS_SIZET(0x80000000) == 31) ? 0 : 0x100;
	rv += (GPCL_DETAIL_TLSF_FLS_SIZET(0x100000000) == 32) ? 0 : 0x200;
	rv += (GPCL_DETAIL_TLSF_FLS_SIZET(0xffffffffffffffff) == 63) ? 0 : 0x400;
#endif
...
{
	return GPCL_DETAIL_TLSF_CAST(pool_t, (char*)tlsf + tlsf_size());
}
...
void* tlsf_memalign(tlsf_t tlsf, size_t align, size_t size)
{
	...
	*/
	...
	if (block)
	{
		...
		size_t gap = GPCL_DETAIL_TLSF_CAST(size_t,
			GPCL_DETAIL_TLSF_CAST(tlsfptr_t, aligned) - GPCL_DETAIL_TLSF_CAST(tlsfptr_t, ptr));

		/* If gap size is too small, offset to next aligned boundary. */
		if (gap && gap < gap_minimum)
		{
			const size_t gap_remain = gap_minimum - gap;
			const size_t offset = GPCL_DETAIL_TLSF_MAX(gap_remain, align);
			const void* next_aligned = GPCL_DETAIL_TLSF_CAST(void*,
				GPCL_DETAIL_TLSF_CAST(tlsfptr_t, aligned) + offset);

			aligned = align_ptr(next_aligned, align);
			gap = GPCL_DETAIL_TLSF_CAST(size_t,
				GPCL_DETAIL_TLSF_CAST(tlsfptr_t, aligned) - GPCL_DETAIL_TLSF_CAST(tlsfptr_t, ptr));
		}
	...
	else
	{
		...
		*/
		if (adjust > cursize && (!block_is_free(next) || adjust > combined))
		{
			...
			if (p)
			{
				const size_t minsize = GPCL_DETAIL_TLSF_MIN(cursize, size);
